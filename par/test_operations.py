...
class TestGitOperations:
    ...
    def test_checkout_worktree_with_remote_fetch(self, mock_run_cmd, mock_repo_root):
        ...
        assert mock_run_cmd.call_count == 2
        mock_run_cmd.assert_any_call(
            ["git", "fetch", "upstream"], cwd=Path("/repo"), suppress_output=True
        )
        mock_run_cmd.assert_any_call(
...
class TestTmuxOperations:
    ...
    @patch("os.getenv")
    @patch("par.operations.run_cmd")
    def test_open_tmux_session_outside_tmux(
        self, mock_run_cmd, mock_getenv, mock_check_tmux
    ):
        ...
        with patch("os.execvp") as mock_execvp:
            ...
            mock_execvp.assert_called_once_with(
                "tmux", ["tmux", "attach-session", "-t", "test-session"]
            )

    @patch("par.operations._check_tmux")
    @patch("os.getenv")
    @patch("par.operations.run_cmd")
    def test_open_tmux_session_inside_tmux(
        self, mock_run_cmd, mock_getenv, mock_check_tmux
    ):
...
class TestWorkspaceOperations:
    ...
    def test_create_workspace_worktree_with_base(self, mock_run_cmd):
        ...
        operations.create_workspace_worktree(
            repo_path, "feature-branch", worktree_path, "develop"
        )
    ...
    def test_create_workspace_tmux_session(self, mock_create_session, mock_check_tmux):
        ...
        mock_create_session.assert_called_once_with(
            "workspace-session", expected_workspace_root
        )
...
class TestControlCenter:
    ...
    def test_open_control_center_success(
        ...
        mock_create_session.assert_called_once_with(
            "par-repo-abc1-cc", Path("/tmp/worktree1")
        )
    ...
    @patch("par.operations.tmux_session_exists")
    @patch("par.operations.open_tmux_session")
    def test_open_control_center_existing(
        self,
        mock_open,
        mock_session_exists,
        mock_get_session_name,
...
# Test file for git and tmux operations
