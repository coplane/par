...
class TestWorkspaceStateManagement:
    ...
    def test_get_workspace_sessions_empty(self, mock_cwd, mock_workspace_state_manager):
        ...
        assert sessions == {}
        mock_workspace_state_manager.get_scoped_data.assert_called_once_with("/workspace")

    @patch('pathlib.Path.cwd')
    def test_get_workspace_sessions_with_data(self, mock_cwd, mock_workspace_state_manager, sample_workspace_data):
        """Test getting existing workspace sessions."""
        mock_cwd.return_value = Path("/workspace")
        mock_workspace_state_manager.get_scoped_data.return_value = sample_workspace_data
    ...
    @patch('pathlib.Path.cwd')
    def test_update_workspace_sessions(self, mock_cwd, mock_workspace_state_manager, sample_workspace_data):
        """Test updating workspace session data."""
        mock_cwd.return_value = Path("/workspace")
        
        workspace._update_workspace_sessions(Path("/workspace"), sample_workspace_data)
        
        mock_workspace_state_manager.update_scoped_data.assert_called_once_with("/workspace", sample_workspace_data)
...
class TestWorkspaceHelperFunctions:
    ...
    def test_prepare_workspace_repos_auto_detect(self, mock_detect_repos):
        ...
        repo_names, repo_paths = workspace._prepare_workspace_repos(None, Path("/workspace"))
    ...
    def test_validate_workspace_creation_success(self, mock_tmux_exists, mock_get_sessions):
        """Test successful workspace creation validation."""
        mock_get_sessions.return_value = {}
        mock_tmux_exists.return_value = False
        
        # Should not raise exception
        workspace._validate_workspace_creation("new-workspace", Path("/workspace"), "session-name")

    @patch('par.workspace._get_workspace_sessions')
    def test_validate_workspace_creation_duplicate_label(self, mock_get_sessions, sample_workspace_data):
        """Test error when workspace label already exists."""
        mock_get_sessions.return_value = sample_workspace_data
        
        with pytest.raises((SystemExit, Exception)):
            workspace._validate_workspace_creation("feature-auth", Path("/workspace"), "session-name")

    @patch('par.workspace._get_workspace_sessions')
    @patch('par.workspace.operations.tmux_session_exists')
    def test_validate_workspace_creation_tmux_conflict(self, mock_tmux_exists, mock_get_sessions):
        """Test error when tmux session already exists."""
        mock_get_sessions.return_value = {}
        mock_tmux_exists.return_value = True
        
        with pytest.raises((SystemExit, Exception)):
            workspace._validate_workspace_creation("new-workspace", Path("/workspace"), "existing-session")

    @patch('par.workspace.utils.get_workspace_worktree_path')
    @patch('par.workspace.operations.create_workspace_worktree')
    def test_create_workspace_worktrees_success(self, mock_create_worktree, mock_get_path):
        """Test successful workspace worktree creation."""
        mock_get_path.side_effect = lambda workspace_root, label, repo_name, branch: \
            Path(f"/tmp/workspaces/{label}/{repo_name}/{branch}")
    ...
    def test_run_workspace_initialization_with_config(self, mock_send_keys, mock_run_init, mock_load_config):
...
class TestWorkspaceOperations:
    ...
    @patch('pathlib.Path.cwd')
    def test_start_workspace_session_success(self, mock_cwd, mock_get_session_name, mock_send_keys,
                                           mock_display, mock_update_state, mock_run_init,
                                           mock_create_tmux, mock_create_worktrees, 
                                           mock_validate, mock_prepare_repos):
        """Test successful workspace session creation."""
        mock_cwd.return_value = Path("/workspace")
        mock_get_session_name.return_value = "par-ws-workspace-abc1-feature-auth"
        mock_prepare_repos.return_value = (["frontend", "backend"], [Path("/workspace/frontend"), Path("/workspace/backend")])
        mock_create_worktrees.return_value = [{"repo_name": "frontend"}, {"repo_name": "backend"}]
        ...
        mock_send_keys.assert_called_with("par-ws-workspace-abc1-feature-auth", "par welcome")
    ...
    @patch('par.workspace.utils.get_workspace_session_name')
    @patch('pathlib.Path.cwd')
    def test_start_workspace_session_with_open(self, mock_cwd, mock_get_session_name, mock_open,
                                             mock_send_keys, mock_display, mock_update_state,
                                             mock_run_init, mock_create_tmux, mock_create_worktrees,
                                             mock_validate, mock_prepare_repos):
    ...
    def test_list_workspace_sessions_with_data(self, mock_console, mock_get_sessions, mock_cwd, sample_workspace_data):
    ...
    def test_open_workspace_session_existing(self, mock_open_tmux, mock_tmux_exists, 
                                           mock_get_sessions, mock_cwd, sample_workspace_data):
    ...
    @patch('par.workspace.operations.create_workspace_tmux_session')
    @patch('par.workspace.operations.open_tmux_session')
    def test_open_workspace_session_recreate(self, mock_open_tmux, mock_create_tmux, mock_tmux_exists,
                                           mock_get_sessions, mock_cwd, sample_workspace_data):
        ...
        mock_create_tmux.assert_called_once_with("par-ws-workspace-abc1-feature-auth", repos_data)
        mock_open_tmux.assert_called_once_with("par-ws-workspace-abc1-feature-auth")
    ...
    @patch('par.workspace._update_workspace_sessions')
    def test_remove_workspace_session_success(self, mock_update_sessions, mock_delete_branch,
                                            mock_remove_worktree, mock_kill_tmux,
                                            mock_get_sessions, mock_cwd, sample_workspace_data):
    ...
    def test_remove_all_workspace_sessions_confirmed(self, mock_input, mock_remove_session,
                                                   mock_get_sessions, mock_cwd, sample_workspace_data):
    ...
    def test_remove_all_workspace_sessions_cancelled(self, mock_input, mock_get_sessions, mock_cwd, sample_workspace_data):
...
class TestWorkspaceIDEIntegration:
    ...
    def test_open_workspace_in_vscode(self, mock_run_cmd, mock_save_file, mock_get_sessions, 
                                    mock_cwd, sample_workspace_data):
        ...
        mock_run_cmd.assert_called_once_with(["code", "/tmp/feature-auth.code-workspace"])

    @patch('pathlib.Path.cwd')
    @patch('par.workspace._get_workspace_sessions')
    @patch('par.workspace.utils.save_vscode_workspace_file')
    @patch('par.workspace.operations.run_cmd')
    def test_open_workspace_in_cursor(self, mock_run_cmd, mock_save_file, mock_get_sessions,
                                    mock_cwd, sample_workspace_data):
        ...
        mock_run_cmd.assert_called_once_with(["cursor", "/tmp/feature-auth.code-workspace"])
    ...
    @patch('pathlib.Path.cwd')
    @patch('par.workspace._get_workspace_sessions')
    def test_open_workspace_in_unsupported_ide(self, mock_get_sessions, mock_cwd, sample_workspace_data):
    ...
    @patch('par.workspace.utils.save_vscode_workspace_file')
    @patch('par.workspace.operations.run_cmd')
    def test_open_workspace_in_ide_command_failure(self, mock_run_cmd, mock_save_file,
                                                  mock_get_sessions, mock_cwd, sample_workspace_data):
...
class TestWorkspaceErrorHandling:
    ...
    @patch('par.workspace._prepare_workspace_repos')
    @patch('par.workspace._validate_workspace_creation')
    def test_start_workspace_session_validation_failure(self, mock_validate, mock_prepare_repos):
    ...
    @patch('par.workspace._validate_workspace_creation')
    @patch('par.workspace._create_workspace_worktrees')
    def test_start_workspace_session_worktree_creation_failure(self, mock_create_worktrees,
                                                             mock_validate, mock_prepare_repos):
...
if __name__ == "__main__":
    print("✅ Workspace management tests defined")
    print("Run with: pytest par/test_workspace.py -v")