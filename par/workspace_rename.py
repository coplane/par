...
def rename_repo_branch(repo_name: str, new_branch: str) -> None:
    ...
    if not current_workspace:
        ...
        typer.secho(
            "The 'par rename' command only works within workspaces.", fg="red", err=True
        )
        raise typer.Exit(1)
    ...
    if not repo_data:
        ...
        typer.secho(
            f"Error: Repository '{repo_name}' not found in workspace '{workspace_label}'.",
            fg="red",
            err=True,
        )
        typer.secho(
            f"Available repositories: {', '.join(available_repos)}", fg="yellow"
        )
        raise typer.Exit(1)
    ...
    if not worktree_path.exists():
        typer.secho(
            f"Error: Worktree path '{worktree_path}' does not exist.",
            fg="red",
            err=True,
        )
        raise typer.Exit(1)
    ...
    console.print(
        f"[cyan]Renaming branch for repository '[bold]{repo_name}[/bold]'[/cyan]"
    )
    ...
    try:
        ...
        existing_branches = operations.run_cmd(
            ["git", "branch", "--list", new_branch],
            cwd=worktree_path,
            suppress_output=True,
        )
        if existing_branches.stdout.strip():
            typer.secho(
                f"Error: Branch '{new_branch}' already exists in repository '{repo_name}'",
                fg="red",
                err=True,
            )
            raise typer.Exit(1)
        ...
        console.print("[green]✅ Successfully renamed git branch[/green]")
    ...
    try:
        ...
        console.print("[green]✅ Updated Par workspace state[/green]")

    except Exception as e:
        typer.secho(f"Error updating workspace state: {e}", fg="red", err=True)
        typer.secho(
            "The git branch was renamed, but Par's state may be inconsistent.",
            fg="yellow",
        )
        raise typer.Exit(1)

    console.print()
    console.print("[bold green]Branch rename complete![/bold green]")
    console.print(
        f"Repository '[cyan]{repo_name}[/cyan]' is now on branch '[green]{new_branch}[/green]'"
    )
