...
class TestRepoSessionManagement:
    ...
    def test_get_repo_sessions_empty(self, mock_git_repo, mock_state_manager):
        ...
        assert sessions == {}
        mock_state_manager.get_scoped_data.assert_called_once_with(str(Path("/tmp/test-repo").resolve()))

    def test_get_repo_sessions_with_data(self, mock_git_repo, mock_state_manager, sample_session_data):
    ...
    def test_update_repo_sessions(self, mock_git_repo, mock_state_manager, sample_session_data):
        """Test updating session data."""
        core._update_repo_sessions(sample_session_data)
        
        mock_state_manager.update_scoped_data.assert_called_once_with(str(Path("/tmp/test-repo").resolve()), sample_session_data)

    def test_update_repo_sessions_empty_cleanup(self, mock_git_repo, mock_state_manager):
        """Test that empty session data is cleaned up."""
        core._update_repo_sessions({})
        
        mock_state_manager.update_scoped_data.assert_called_once_with(str(Path("/tmp/test-repo").resolve()), {})
...
class TestSessionOperations:
    ...
    def test_start_session_success(self, mock_init, mock_utils, mock_ops, mock_git_repo, mock_state_manager):
        """Test successful session creation."""
        # Setup mocks
        mock_utils.get_worktree_path.return_value = Path("/tmp/par/worktrees/abc123/test-session")
        mock_utils.get_tmux_session_name.return_value = "par-test-repo-abc1-test-session"
    ...
    def test_start_session_duplicate_label(self, mock_git_repo, mock_state_manager, sample_session_data):
    ...
    @patch('par.core.operations')
    @patch('par.core.utils')
    def test_start_session_worktree_conflict(self, mock_utils, mock_ops, mock_git_repo, mock_state_manager):
    ...
    @patch('par.core.operations')
    @patch('par.core.utils')
    def test_start_session_tmux_conflict(self, mock_utils, mock_ops, mock_git_repo, mock_state_manager):
    ...
    @patch('par.core.utils')
    @patch('par.core.initialization')
    def test_start_session_with_initialization(self, mock_init, mock_utils, mock_ops, mock_git_repo, mock_state_manager):
        ...
        mock_init.run_initialization.assert_called_once_with(
            mock_config, "par-test-repo-abc1-test-session", Path("/tmp/par/worktrees/abc123/test-session")
        )

    @patch('par.core.operations')
    def test_remove_session_success(self, mock_ops, mock_git_repo, mock_state_manager, sample_session_data):
        ...
        mock_ops.kill_tmux_session.assert_called_once_with("par-test-repo-abc1-test-session")
        mock_ops.remove_worktree.assert_called_once_with(Path("/tmp/par/worktrees/abc123/test-session"))
    ...
    def test_remove_all_sessions_confirmed(self, mock_input, mock_ops, mock_git_repo, mock_state_manager, sample_session_data):
        ...
        mock_state_manager.update_scoped_data.assert_called_with(str(Path("/tmp/test-repo").resolve()), {})

    @patch('typer.confirm')
    def test_remove_all_sessions_cancelled(self, mock_confirm, mock_git_repo, mock_state_manager, sample_session_data):
    ...
    @patch('par.core.operations')
    def test_open_session_existing(self, mock_ops, mock_git_repo, mock_state_manager, sample_session_data):
        ...
        mock_ops.open_tmux_session.assert_called_once_with("par-test-repo-abc1-test-session")

    @patch('par.core.operations')
    def test_open_session_recreate(self, mock_ops, mock_git_repo, mock_state_manager, sample_session_data):
    ...
    @patch('par.core.operations')
    def test_send_command_specific_session(self, mock_ops, mock_git_repo, mock_state_manager, sample_session_data):
        """Test sending command to specific session."""
        mock_state_manager.get_scoped_data.return_value = sample_session_data
        
        core.send_command("test-session", "echo hello")
        
        mock_ops.send_tmux_keys.assert_called_once_with("par-test-repo-abc1-test-session", "echo hello")

    @patch('par.core.operations')
    def test_send_command_all_sessions(self, mock_ops, mock_git_repo, mock_state_manager):
        ...
        mock_ops.send_tmux_keys.assert_any_call("par-test-repo-abc1-session1", "git status")
        mock_ops.send_tmux_keys.assert_any_call("par-test-repo-abc1-session2", "git status")
    ...
    def test_list_sessions_empty(self, mock_workspace_sessions, mock_git_repo, mock_state_manager):
    ...
    def test_list_sessions_with_data(self, mock_console, mock_ops, mock_git_repo, mock_state_manager, sample_session_data):
    ...
    @patch('par.core.operations')
    def test_open_control_center_success(self, mock_ops, mock_git_repo, mock_state_manager, sample_session_data):
    ...
    @patch('par.workspace._get_workspace_sessions')
    def test_open_control_center_no_sessions(self, mock_workspace_sessions, mock_git_repo, mock_state_manager):
...
class TestCheckoutOperations:
    ...
    @patch('par.core.operations')
    @patch('par.core.utils')
    def test_checkout_session_existing_branch(self, mock_utils, mock_ops, mock_checkout, mock_git_repo, mock_state_manager):
        """Test checking out existing branch."""
        mock_checkout.return_value = ("existing-branch", CheckoutStrategy(ref="existing-branch", is_pr=False))
        
        mock_utils.get_worktree_path.return_value = Path("/tmp/par/worktrees/abc123/existing-branch")
        mock_utils.get_tmux_session_name.return_value = "par-test-repo-abc1-existing-branch"
        mock_ops.tmux_session_exists.return_value = False
    ...
    @patch('par.core.operations')
    @patch('par.core.utils')
    def test_checkout_session_pr(self, mock_utils, mock_ops, mock_checkout, mock_git_repo, mock_state_manager):
        """Test checking out PR."""
        mock_checkout.return_value = ("pr-123", CheckoutStrategy(ref="origin/pull/123/head", is_pr=True))
        
        mock_utils.get_worktree_path.return_value = Path("/tmp/par/worktrees/abc123/pr-123")
    ...
    @patch('par.core.checkout.parse_checkout_target')
    def test_checkout_session_custom_label(self, mock_checkout, mock_git_repo, mock_state_manager):
        """Test checkout with custom label."""
        mock_checkout.return_value = ("custom-label", CheckoutStrategy(ref="develop"))
        
        with patch('par.core.operations') as mock_ops, \
             patch('par.core.utils') as mock_utils:
            
            mock_utils.get_worktree_path.return_value = Path("/tmp/par/worktrees/abc123/custom-label")
            mock_utils.get_tmux_session_name.return_value = "par-test-repo-abc1-custom-label"
            mock_ops.tmux_session_exists.return_value = False
...
class TestValidationAndErrorHandling:
    ...
    def test_session_validation_invalid_characters(self, mock_git_repo, mock_state_manager):
    ...
    @patch('par.core.operations')
    def test_error_recovery_partial_cleanup(self, mock_ops, mock_git_repo, mock_state_manager):
