...
"""
    ...
    console.print("[bold magenta]the agentic terminal multiplexer[/bold magenta]")
    console.print()
    ...
    try:
        ...
        if contexts_info["total_contexts"] > 0:
            if contexts_info["is_inside_workspace"]:
                ...
                console.print(f"🌟 [bold green]Active Workspace:[/bold green] [cyan]{current_workspace['label']}[/cyan]")
                console.print()
                ...
                console.print("  [bright_blue]par claude --agent[/bright_blue]    # AI agent mode")
                console.print("  [bright_blue]par cursor[/bright_blue]           # Open in Cursor")
            else:
                # Show all contexts (ls-style)
                console.print(f"🌟 [bold green]Development Contexts[/bold green] ({contexts_info['total_contexts']} active)")
                console.print()
                ...
                console.print(table)
                console.print()
                console.print("[dim]Run [cyan]par --help[/cyan] to see all available commands[/dim]")
        else:
            console.print("🌟 [bold green]Ready to Start![/bold green]")
            console.print()
            console.print("[dim]No active development contexts found[/dim]")
            console.print("[dim]Create one with: [cyan]par start <name>[/cyan] or [cyan]par ws start <name>[/cyan][/dim]")
            console.print()
            console.print("[dim]Run [cyan]par --help[/cyan] to see all available commands[/dim]")
            ...
            try:
                ...
                for workspace_root, workspace_data in all_workspaces.items():
                    for ws_label, ws_info in workspace_data.items():
                        ...
                        for repo_data in ws_info.get("repos", []):
                            worktree_path = repo_data.get("worktree_path", "")
                            if worktree_path and current_path_str.startswith(str(Path(worktree_path).parent)):
                                workspace_in_scope = True
                                break
                        ...
                        if workspace_in_scope:
                            repo_names = [r["repo_name"] for r in ws_info.get("repos", [])]
